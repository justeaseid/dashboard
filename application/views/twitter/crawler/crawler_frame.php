...
//                        echo "<td>" . date("d F Y h:i:s A", strtotime($row["last_update"])+21600) . "</td>";
                        echo "<td>" . date("d F Y h:i:s A", strtotime($row["last_update"])) . "</td>";
                        echo '<td><button id="delete' . $i . '" type="" class="btn btn-fourth" onclick="delete_twitter(' . $row['cw_id'] . ');">Delete</button></td>';
                        echo "</tr>";
                        $i++;
                    }
                }
...
<script>
    ...
    function delete_twitter(twitter_id) {
        top.location = "<?php echo base_url('/twitter/crawler_delete/' . $url . '/1') ?>/" + twitter_id;
    }
</script>