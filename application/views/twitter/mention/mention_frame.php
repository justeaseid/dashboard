            ...
            <tbody>
                ...
                if ($json_result->num_rows() > 0) {
                    ...
                    foreach ($json_result->result_array() as $row) {
                        ...
                        $message = preg_replace('/[^A-Z a-z 0-9 \!\?\@\%\&\(\)\:\\\,\-\#\$\.]/', '', $row['mn_text']);
                        echo "<td>" . $message . "</td>";
                        echo "<td><b>@" . $row['mn_from'] . "</b></td>";
//                        echo "<td>" . date("d F Y", strtotime($row['ps_created_time'])) . "</td>";
                        echo "<td><div class='fa fa-retweet'> " . $row['mn_retweet'] . "</div></td>";
                        echo "<td><div class='fa fa-heart'> " . $row['mn_favorite'] . "</td>";
//                        $surl = base_url('/twitter/vcomment/' . $url . '/1/' . $row["ps_id"]);
//                        echo '<td><a class="btn btn-primary fa fa-comment" href="' . $surl . '"></a></td>';
                        $url = TWITTER . $row["mn_from"] . "/status/" . $row["mn_id"];
