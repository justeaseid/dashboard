            ...
            <tbody>
                ...
                if ($json_result->num_rows() > 0) {
                    ...
                    foreach ($json_result->result_array() as $row) {
                        echo "<tr>";
                        echo "<td><b>@" . $row['tw_from'] . "</b></td>";
                        $message = preg_replace('/[^A-Z a-z 0-9 \!\?\@\%\&\(\)\:\\\,\-\#\$\.]/', '', $row['tw_text']);
                        echo "<td>" . $message . "</td>";
//                        echo "<td>" . date("d F Y", strtotime($row['ps_created_time'])) . "</td>";
                        echo "<td><div class='fa fa-retweet'> " . $row['tw_retweet'] . "</div></td>";
                        echo "<td><div class='fa fa-heart'> " . $row['tw_favorite'] . "</div></td>";
//                        $surl = base_url('/twitter/vcomment/' . $url . '/1/' . $row["ps_id"]);
//                        echo '<td><a class="btn btn-primary fa fa-comment" href="' . $surl . '"></a></td>';
                        $url = TWITTER . $row["tw_from"] . "/status/" . $row["tw_id"];
